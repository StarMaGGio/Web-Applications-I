    ...
    <>
      <Container fluid className="d-grid gap-1 align-items-left">
        ...
        <Table>
          <tbody>
            {filteredFilms.map((f) => (
              <FilmRow key={f.id} f={f} deleteFilm={props.deleteFilm} setMode={props.setMode} setSelectedFilm={props.setSelectedFilmId} navigate={navigate}></FilmRow>
            ))}
          </tbody>
        </Table>
      </Container>
        ...
        <div className="d-flex justify-content-end align-items-center gap-3">
          ...
          <div>
            <Button variant="light" onClick={()=>{
                  props.setMode("edit")
                  props.setSelectedFilm(props.f.id)
                  props.navigate('/edit-film')
                }
              }>
